...
/**
 * A SimpleFunctionCollection is a collection of static functions.
...
public class SimpleFunctionCollection extends HashMap<FunctionReference,StaticFunction> implements FunctionCollection{
    ...
    public boolean collect(FunctionReference file,ArrayList<CompilationProblem> errList){
        ...
        if (program instanceof Script){
            System.err.println("The tamer does not suport scripts at this point."); //TODO
            return false;
        ...
        //turn functions into static functions, and go through their function references recursively
        ...
        for (Function functionAst : functionList.getFunctions()){
            ...
            Context context = fileEnvironment.getContext(functionAst, file.getFile());
            StaticFunction function = new StaticFunction(functionAst, ref, context);
    ...
    private boolean resolveFunctionsAndCollect(StaticFunction function,ArrayList<CompilationProblem> errList){
        ...
        for (String otherName : function.getCalledFunctions().keySet()){            
        	FunctionReference ref = function.getCalledFunctions().get(otherName);
        	if (ref == null){
